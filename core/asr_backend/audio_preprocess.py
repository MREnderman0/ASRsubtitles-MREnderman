        ...
        for word in segment['words']:
            ...
            if 'start' not in word and 'end' not in word:
                if all_words:
                    ...
                    row_segment_text = segment_text if not segment_text_written else ''
                    word_dict = {
                        'text': word["word"],
                        'start': all_words[-1]['end'],
                        'end': all_words[-1]['end'],
                        'speaker_id': speaker_id,
                        'segment_text': row_segment_text,
                    }
                    all_words.append(word_dict)
                    segment_text_written = True
                else:
...
def save_results(df: pd.DataFrame):
    ...
    if 'segment_text' in df.columns:
        df['segment_text'] = df['segment_text'].fillna('').astype(str)
