...
def transcribe():
    ...
    all_results = []
    asr_backend = load_key("asr.backend") or "whisperx"
    runtime = load_key("whisper.runtime")
    if asr_backend == "qwen3":
        from core.asr_backend.qwen3_asr import transcribe_audio_qwen3 as ts
        rprint("[cyan]Transcribing audio with Qwen3-ASR local model...[/cyan]")
    elif runtime == "local":
